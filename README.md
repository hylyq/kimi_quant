...
- 🛡️ **多层防护**：Firefox TLS 指纹伪装 + 七层风控 + 异常熔断 + 风控拒绝反馈修正（LLM 一次更正机会）+ SL/TP 链上验证 + WebSocket 自恢复
...
├─────────────────────────────────────────────────────┤
│ Step 1.5 — 反事实检查（方向性决策前强制执行）           │
│                                                     │
│  输出 LONG/SHORT 之前，反问：                          │
│  "如果我错了 —— 什么价格能证明？" → 那就是止损位        │
│  "如果市场反向剧烈波动 —— 我漏掉了什么信号？"            │
│  "我是在交易看到的证据，还是心里想看到的东西？"           │
│  反事实太强 → 降 confidence 0.1-0.15 或 HOLD          │
└─────────────────────────────────────────────────────┘
```

**关键原则**：Step 0 的修复动作（如恢复丢失的 SL）优先级高于 Step 1 的新开仓动作。如果 Step 0 发现 SL 丢失且市场出现做多信号，LLM 应输出 `["MODIFY_SL", "LONG"]` 而非 `["LONG"]`——先保护仓位，再开新仓。

这项设计通过 **System Prompt + User Prompt Instructions + Judge Prompt** 三层提示词共同强制，不需要额外代码逻辑。在 Debate 模式下，Judge 与辩手共享相同的原始市场数据（通过 `RAW MARKET DATA` 区块），可直接验证辩手引用的价格和指标是否准确。**Step 1.5 反事实检查**要求 LLM 在方向性决策前先自问"如果我错了会怎样"——降低过度自信导致的重仓。**周期间 diff** 自动注入每轮 prompt，标注实际经过的时间（因为 LLM 自主控制唤醒间隔），让 LLM 聚焦变化而非重新分析全量数据。
...
修正提示词不预设具体修正方案——拒绝原因本身就是最好的指南。提示词只做两件事：
1. 明确区分"可修正参数"（价格、仓位、置信度）和"硬阻断"（熔断激活、日内回撤超限）
2. 禁止重复犯错
