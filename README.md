...
- 🎯 **四步强制决策**：Step 0 审视持仓 → Step 1 市场分析 → Step 1.5 强制反事实检查清单 → 输出信号
- ⏰ **时区/日历感知**：自动注入 UTC/北京时间、星期、交易时段（亚/欧/美盘），周末低流动性自动警告
- 📌 **持仓记忆**：LLM 看到原始开仓逻辑、持仓时长、最大有利/不利偏移（MAE/MFE），真正验证 thesis 是否成立
- 🔄 **决策反馈闭环**：上周期决策→结果自动注入下周期，"上次说等回踩确认 → 现在回踩了"的闭环
...
- 📊 **周期间 diff + 波动率状态**：自动对比上轮数据 + 分类 HIGH/NORMAL/LOW 波动率环境，止损宽度动态指导
- 💬 **两种策略 + 反驳轮**：Single / Debate（三 Agent 辩论 + 可选反驳轮 + Judge 交叉验证裁决），辩手可感知账户持仓状态
...
- 🧮 **期望值框架**：R:R 隐含盈亏平衡胜率自动计算，LLM 知道 confidence 必须超过多少才有正期望
