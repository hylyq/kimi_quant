...
class OrderMonitor:
    ...
    def _parse_order_update(self, data: Any) -> list[OrderEvent]:
        ...
        The WebSocket frequently batches MULTIPLE order updates in one
        message — ALL of them are returned (a single return on the first
        entry would silently drop the rest, e.g. a flip's CLOSE+SHORT fills).
        """
    ...
    def _parse_fill_update(self, data: Any) -> list[OrderEvent]:
        ...
        The subscription delivers a batch of fills per message — all of them
        are returned so none are dropped.
        """
        if not isinstance(data, dict):
            return []
